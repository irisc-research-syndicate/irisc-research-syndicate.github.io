...
fields_decoders = {
    ...
    "stoff16": lambda w: (((w >> 16) & 0x1f) << 11) | (w & 0x7ff),
}

opcodes = defaultdict(lambda: lambda fs: "unk.{op:02x} r{rs}, r{rd}, r{rt}, {imm16:#06x}".format(**fs))
opcodes[0x00] = lambda fs: "add r{rd}, r{rs}, {imm16}".format(**fs)
opcodes[0x1b] = lambda fs: "st.d r{rt}, r{rs}, {stoff16:#06x}".format(**fs)
opcodes[0x1c] = lambda fs: "st.d! r{rt}, r{rs}, {stoff16:#06x}".format(**fs)
